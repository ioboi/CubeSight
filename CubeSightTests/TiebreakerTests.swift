  ...
  func testMatchWinPercentage() throws {
    ...
    #expect(
      (tournament.performance[players[0]]?.matchWinRate ?? 0)
        .isApproximatelyEqual(to: 16.0 / 24.0),
      "Player with 5-2-1 record should have 5*3+1 / 8*3 = 0.667 match-win percentage"
    )
    #expect(
      (tournament.performance[players[1]]?.matchWinRate ?? 0)
